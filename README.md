...
Found an `auth/unauthorized-domain` or redirect-flow cause this tool doesn't catch, or a check that flags something that's actually fine? Open an issue: https://github.com/AndryRoby/firebase-auth-domain-doctor/issues, or write to support@arling.sk. Please redact anything sensitive (Firebase project IDs, API keys, real domains) before posting; issues are public.
